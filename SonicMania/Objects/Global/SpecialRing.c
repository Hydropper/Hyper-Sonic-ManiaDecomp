...
void SpecialRing_Draw(void)
{
    ...
    SpecialRing->colorChange1 = SpecialRing->colorChange;
    SpecialRing->colorChange1 /= 4;
    ...
    if (self->state == SpecialRing_State_Flash) {
        ...
        if (self->enabled)
            if (SpecialRing->chaosEmeralds >= 255) {
                switch (SpecialRing->colorChange1) {
                    case 0:
                        RSDK.AddModelTo3DScene(SpecialRing->modelIndex, SpecialRing->sceneIndex, S3D_SOLIDCOLOR_SHADED_BLENDED, &self->matWorld,
                                               &self->matNormal, 0xF0F000);
                        SpecialRing->colorChange += 1;
                        break;
                    case 1:
                        RSDK.AddModelTo3DScene(SpecialRing->modelIndex, SpecialRing->sceneIndex, S3D_SOLIDCOLOR_SHADED_BLENDED, &self->matWorld,
                                               &self->matNormal, 0xfCD8FC);
                        SpecialRing->colorChange += 1;
                        break;
                    case 2:
                        RSDK.AddModelTo3DScene(SpecialRing->modelIndex, SpecialRing->sceneIndex, S3D_SOLIDCOLOR_SHADED_BLENDED, &self->matWorld,
                                               &self->matNormal, 0xB4D8FC);
                        SpecialRing->colorChange += 1;
                        break;
                    case 3:
                        RSDK.AddModelTo3DScene(SpecialRing->modelIndex, SpecialRing->sceneIndex, S3D_SOLIDCOLOR_SHADED_BLENDED, &self->matWorld,
                                               &self->matNormal, 0x90FC90);
                        SpecialRing->colorChange += 1;
                        break;
                    case 4:
                        RSDK.AddModelTo3DScene(SpecialRing->modelIndex, SpecialRing->sceneIndex, S3D_SOLIDCOLOR_SHADED_BLENDED, &self->matWorld,
                                               &self->matNormal, 0xD8fC6C);
                        SpecialRing->colorChange += 1;
                        break;
                    case 5:
                        RSDK.AddModelTo3DScene(SpecialRing->modelIndex, SpecialRing->sceneIndex, S3D_SOLIDCOLOR_SHADED_BLENDED, &self->matWorld,
                                               &self->matNormal, 0xFCD86C);
                        SpecialRing->colorChange = 0;
                        break;
                }
            }
            else {
...
void SpecialRing_State_Idle(void)
{
    ...
    saveRAM->chaosEmeralds = 255;
    SpecialRing->chaosEmeralds = saveRAM->chaosEmeralds;
